			...
			<div class="video_height_holder">
				...
				{/if}
					...
					<a href="{$aVideo.link}" class="js_video_title_{$aVideo.video_id}">
						...
						<img src="https://c197913.ssl.cf1.rackcdn.com/{$aVideo.vidly_url_id}/thumbnail_1.jpg" alt="{$aVideo.title|clean}" style="max-width:120; max-height:90px;" class='js_mp_fix_width video_image_border' />
						{else}
...
{php}
//vd($this->_aVars['aVideo']);
{/php}
<div class="like_count">
    {if Phpfox::isUser() && Phpfox::isModule('like')}
    <div class="js_comment_like_holder" id="js_feed_like_holder_{$aVideo.video_id}">
        <div id="js_like_body_{$aVideo.video_id}" class="like_image like-video">
            {if isset($aVideo.total_like)}
            <a href="{$aVideo.link}" class="row_sub_link js_video_title_{$aVideo.video_id}" id="js_video_title_{$aVideo.video_id}" itemprop="url">{$aVideo.total_like}</a>
            {/if}
        </div>
    </div>
    {/if}
    ...
    {/if}
			...
			<header>
				<h1 itemprop="name"><a href="{$aVideo.link}" class="row_sub_link js_video_title_{$aVideo.video_id}" id="js_video_title_{$aVideo.video_id}" itemprop="url">{$aVideo.title|clean|shorten:20:'...'|split:20}</a></h1>
			</header>
