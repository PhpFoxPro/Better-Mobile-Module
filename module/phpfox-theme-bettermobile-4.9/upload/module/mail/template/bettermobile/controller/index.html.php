...
<div id="js_message_{if Phpfox::getParam('mail.threaded_mail_conversation')}{$aMail.thread_id}{else}{$aMail.mail_id}{/if}" class="mail_holder{if !$bIsSentbox && !$bIsTrash && $aMail.viewer_is_new} mail_is_new{/if}">
	...
	<div class="mail_content">
		...
		<a href="{url link='mail.thread' id=$aMail.thread_id}{if $bIsSentbox}view_sent/{/if}" class="mail_link">
			{foreach from=$aMail.users name=mailusers item=aMailUser}{if $phpfox.iteration.mailusers <= 3}{if count($aMail.users) == $phpfox.iteration.mailusers && count($aMail.users) > 1} &amp; {else}{if $phpfox.iteration.mailusers != 1 && count($aMail.users) != 2}, {/if}{/if}{$aMailUser.full_name|clean|shorten:15:'...'}{/if}{/foreach}
		</a>
		{else}
		<a href="{url link='mail.view' id=$aMail.mail_id}" class="mail_link">
			{if $aMail.parent_id}{phrase var='mail.re'}: {/if}{$aMail.subject|clean|shorten:20:'...'}
		</a>
		{/if}
		...
		<div class="mail_preview">
			{if isset($aMail.last_user_id) && $aMail.last_user_id == Phpfox::getUserId()}{img theme='layout/arrow_left.png' class='v_middle'} {/if}{$aMail.preview|clean|shorten:20:'...'|cleanbb}
		</div>
		{/if}
        {if !$bIsInLegacyView}
