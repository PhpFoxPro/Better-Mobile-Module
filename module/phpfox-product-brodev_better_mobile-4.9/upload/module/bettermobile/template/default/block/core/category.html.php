...
?>
{php}
$sUrlReplace = Phpfox::getParam('core.path') . "mobile/" ;
foreach($this->_aVars['aCategories'] as $iKey => $aCategory) {
    if (!preg_match('[mobile//]', $aCategory['url'])) {
        $this->_aVars['aCategories'][$iKey]['url'] = preg_replace('['. Phpfox::getParam('core.path') .']', $sUrlReplace, $aCategory['url']);
    }
}
{/php}
